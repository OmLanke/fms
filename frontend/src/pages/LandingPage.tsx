  ...
  return (
    <main>
      ...
      <section className="relative isolate overflow-hidden border-b border-white/[0.06]">
        {/* Background layers */}
        <div className="absolute inset-0 -z-20 bg-[radial-gradient(ellipse_80%_60%_at_50%_-20%,rgba(34,211,238,0.15),transparent)]" />
        <div className="absolute inset-0 -z-20 dot-grid opacity-40" />
        <div className="absolute -z-10 top-0 left-1/4 h-72 w-72 rounded-full bg-primary/5 blur-3xl" />
        <div className="absolute -z-10 top-10 right-1/4 h-48 w-48 rounded-full bg-accent/10 blur-3xl" />

        {/* Floating decorative pills */}
        <div className="absolute top-16 left-[8%] hidden xl:flex items-center gap-1.5 rounded-2xl border border-primary/20 bg-primary/5 px-3 py-1.5 text-xs font-mono text-primary/70 animate-float backdrop-blur-sm">
          <span className="h-1.5 w-1.5 rounded-full bg-primary/60" />
          redis://lock:seat
        </div>
        <div className="absolute top-24 right-[7%] hidden xl:flex items-center gap-1.5 rounded-2xl border border-amber-500/20 bg-amber-500/5 px-3 py-1.5 text-xs font-mono text-amber-400/80 animate-float backdrop-blur-sm" style={{ animationDelay: '1s' }}>
          <span className="h-1.5 w-1.5 rounded-full bg-amber-400/60" />
          booking.confirmed
        </div>
        <div className="absolute bottom-20 left-[12%] hidden xl:flex items-center gap-1.5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-3 py-1.5 text-xs font-mono text-emerald-400/80 animate-float backdrop-blur-sm" style={{ animationDelay: '2s' }}>
        ...
        <div ref={heroRef} className="mx-auto flex w-full max-w-7xl flex-col items-center gap-7 px-4 py-20 text-center md:px-6 md:py-28">
          <div className="hero-item flex items-center gap-2 rounded-full border border-primary/20 bg-primary/8 px-4 py-1.5 text-xs font-semibold text-primary backdrop-blur-sm">
          ...
          <h1 className="hero-item max-w-4xl text-5xl font-black leading-[1.05] tracking-[-0.03em] md:text-7xl">
            ...
            <span className="text-gradient">Without Seat Collisions</span>
          </h1>
          ...
          <div className="hero-item flex flex-wrap justify-center gap-3">
            <a href="#events">
              <Button size="lg" className="animate-pulse-glow gap-2 font-bold px-8">
...
function EventCard({ event }: { event: Event }) {
  return (
    <div className="event-card group relative rounded-2xl border border-white/8 bg-card overflow-hidden transition-all duration-300 hover:border-primary/25 hover:-translate-y-0.5 hover:shadow-[0_8px_40px_rgba(34,211,238,0.08)]">
      {/* Top accent bar */}
      <div className="h-0.5 w-full bg-gradient-to-r from-primary/0 via-primary/50 to-primary/0 opacity-0 group-hover:opacity-100 transition-opacity" />

      <div className="p-6 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-bold tracking-tight leading-snug truncate">{event.name}</h3>
        ...
        <div className="flex items-center justify-between pt-1 border-t border-white/6">
          <div>
            ...
            <div className="text-xl font-black text-gradient">{formatCurrency(event.price)}</div>
          </div>
